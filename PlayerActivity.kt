...
import com.Capstone.musicPlayer.API.DataItem
import com.Capstone.musicPlayer.databinding.ActivityPlayerBinding
import com.Capstone.musicPlayer.databinding.AudioBoosterBinding
...
class PlayerActivity : AppCompatActivity(), ServiceConnection, MediaPlayer.OnCompletionListener {
    ...
    companion object {
        lateinit var musicListPA : ArrayList<DataItem>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(intent.data?.scheme.contentEquals("content")){
            ...
            Glide.with(this)
                .load(getImgArt(musicListPA[songPosition].link!!))
        ...
        binding.shareBtnPA.setOnClickListener {
            ...
            shareIntent.putExtra(Intent.EXTRA_STREAM, Uri.parse(musicListPA[songPosition].preview))
            startActivity(Intent.createChooser(shareIntent, "Sharing Music File!!"))
            
        }
        binding.favouriteBtnPA.setOnClickListener {
            fIndex = favouriteChecker(musicListPA[songPosition].id.toString())
            if(isFavourite){
                isFavourite = false
...
//        val img = getImgArt(musicListPA[songPosition].artist!!.picture!!)
        val image =
    ...
    private fun createMediaPlayer(){
        try {
            ...
            musicService!!.mediaPlayer!!.setDataSource(musicListPA[songPosition].preview)
            ...
            nowPlayingId = musicListPA[songPosition].id.toString()
            playMusic()
    ...
    override fun onCompletion(mp: MediaPlayer?) {
        ...
        Glide.with(applicationContext)
            .load(musicListPA[songPosition].artist!!.picture)
            .apply(RequestOptions().placeholder(R.drawable.music_player_icon_slash_screen).centerCrop())
    ...
    @RequiresApi(Build.VERSION_CODES.Q)
    private fun getMusicDetails(contentUri: Uri): DataItem{
    ...
    override fun onDestroy() {
        super.onDestroy()
        if(musicListPA[songPosition].id.toString() == "Unknown" && !isPlaying) exitApplication()
    }
    private fun initServiceAndPlaylist(playlist: ArrayList<DataItem>, shuffle: Boolean, playNext: Boolean = false){
